...
We now explore the sensitivity of size on a number of ecological and physiological traits. We model variable oxygen limitation through a set of three scenarios that reflect responses found empirically (\cite{lefevre_are_2016}; Figure \ref{fig:O2_fig}), ranging from an increasing MMR with temperature over most of the range (scenario 1), via a slightly dome-shaped response (scenario 2) to a strongly dome-shaped MMR (scenario 3). 

Other physiological traits explored here are the FAS $\delta$, which determines the cost of active metabolism relative to standard metabolism. FAS ca vary strongly between species but is constant with weight (\cite{lefevre_models_2017, killen_ecological_2016}), and range from approximately 2 to 12, with most values found around 3-6. Here we explore values of 3-5. We explore the sensitivity to metabolic exponents by assuming that the exponent ranges between 0.85 and 0.9, with a base-case of 0.88 (\cite{lefevre_models_2017}).
...
\caption[Activity, feeding and maximum length as a function of temperature]{Consequences of increasing temperature on activity levels, feeding and maximum weight for an individual weighing 100g. (a) Optimal (dashed line), limiting (dotted line) and realised (solid line) activity as a function of temperature. Realised activity is the smaller of the limiting and optimal levels at each temperature, with activity limited by O2 supply as pictured by dashed lines in b). Optimal activity steadily increases to compensate for rising metabolic costs (dotted line in b) with temperature until limited by oxygen (overlapping supply (dashed) and demand (solid) curves in b). (c) Rising activity levels keeps feeding levels constant (solid line in c).  Mortality levels (dashed) increase as activity increases with temperature, while available energy (carbon, dotted line) for growth and reproduction peaks and decreases. Note that energy is only limited by oxygen-limited activity levels at low temperatures in scenario 3. The rate of change of available energy relative to mortality drives the adult size, which declines near linearly in log-space without oxygen limitations, and lower and upper limits imposed as oxygen limits activity and energy acquisition.}
\label{fig:act_fig}
\end{figure}

Increasing temperatures have cascading effects through their influence on both metabolic and ecological rates (Figure \ref{fig:act_fig}). To maintain optimal energy acquisition for growth and reproduction relative to mortality, activity levels generally increase, until they reach limitations imposed by oxygen supply (Figure \ref{fig:act_fig}a). This limit occurs when active metabolic costs equal oxygen supply (Figure \ref{fig:act_fig}b), and are therefore more pronounced for species with strongly dome-shaped maximum-metabolic rates (scenario 3 in Figure \ref{fig:act_fig}).
