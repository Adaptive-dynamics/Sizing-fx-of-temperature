...
\begin{figure}
\centering
<<activity, fig.height=7,fig.width=6>>=
...
lab1 <- as_labeller(c(`M strategy Rising` ='c',
                   `M strategy Domed`  ='a',
                   `P strategy Rising` ='g',
                   `P strategy Domed` = 'e'))

lab2 <- as_labeller(c(`M strategy Rising` ='d',
                   `M strategy Domed`  ='b',
                   `P strategy Rising` ='h',
                   `P strategy Domed` = 'f'))

ggsc <- ggplot(sc_act %>% filter(Dispatch=='M strategy Domed')) + 
...
ggso <- sco  %>% filter(Dispatch=='M strategy Domed') %>%
  ggplot() + 
...
ggplot(sc_acts %>% filter(Dispatch=='P strategy Large')) + 
  geom_line(aes(x=Temperature, y=value, linetype=Activity,col=Activity)) +
  theme_cowplot()+
  scale_color_manual(values = cols)+
  scale_linetype()+
...
ggplot(sc_feed%>% filter(Dispatch=='M strategy Domed')) + 
...
gouts <- parallel::mclapply(scs[2],plot_data_growth_tO2,mc.cores = 2)
...
cowplot::plot_grid(f1,f2,ncol=1,labels = 'auto')
